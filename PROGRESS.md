...
Phase 4 subtask 4.4, remaining part: episode numbering. Add a step to tools/ingest/src/anime.rs that, for each media_item now carrying an anilist external id, fetches the entry's episode list and writes both the seasonal and the absolute number into the episode_numbering table added by migration crates/persistence/migrations/0003_series_episodes.up.sql. The catalogue half of 4.4 is done and committed as 87f15f6.
