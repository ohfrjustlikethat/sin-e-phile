...
Phase 4 subtask 4.4, remaining part: episode numbering. Add a step to tools/ingest/src/anime.rs that, for each media_item carrying an anilist external id (7,383 of them now), fetches the entry's episode list and writes BOTH the seasonal and the absolute number into the episode_numbering table from crates/persistence/migrations/0003_series_episodes.up.sql. The catalogue half of 4.4 is done: 14,737 AniList entries swept, 7,328 matched, recorded in docs/eval-results.md.
