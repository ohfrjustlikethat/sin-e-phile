...
use sinephile_catalogue::matching::{match_title, title_forms, TitleIndex};
use sinephile_catalogue::{normalise, Job};
...
async fn the_candidates_feed_straight_into_the_matcher() {
    ...
    let keys: Vec<String> = forms
        .iter()
        .map(|f| sinephile_catalogue::matching::normalise(f))
        .collect();
