...
// SAFETY: `Mpv` holds a raw `*mut mpv_handle`, which makes it non-Send by default.
// mpv's client API permits a handle to be USED from a different thread than the one
// that created it, provided only one thread drives it at a time. This spike moves
// ownership to a single event-pump thread and never shares it, which satisfies that.
//
// It is NOT Sync, deliberately: two threads calling mpv_wait_event on one handle is
// exactly the case mpv does not allow, and leaving Sync unimplemented makes that
