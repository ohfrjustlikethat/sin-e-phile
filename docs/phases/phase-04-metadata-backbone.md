...
- [ ] **4.10** The embedding artefact producer (ADR-0014). crates/embedding holds the document builder, int8 quantisation and the checksummed format; tools/ingest/src/embed.rs is the resumable producer, with the model and tokenizer checksum-pinned. Format specified in docs/specs/embedding-artefact.md. A full run over the 855,703 core titles is IN PROGRESS - it closes E7.
