...
- [ ] **5.6** fixtures/search/ and the relevance harness. exact-titles.tsv (43 queries), `eval search`, `eval vector` (recall 0.9810 with a --prove control) and `eval embed` (byte-identical agreement, plus --compare for per-document cosine) all exist. semantic-queries.tsv and nDCG@10 are BLOCKED BY B4: a fixture measured against a model that cannot do asymmetric retrieval would record the model's ceiling as the system's score.
